...
def main() -> None:
    ...
    missing_result_trace_id = f"rt_{uuid4().hex}"
    redis_runtime.sync_recovery_trace(
        recovery_trace_id=missing_result_trace_id,
        payload={
            "recovery_trace_id": missing_result_trace_id,
            "run_id": run_id,
            "bot_id": bot_id,
    ...
    runtime.run_once()
    missing_result_trace = redis_runtime.get_recovery_trace(
        recovery_trace_id=missing_result_trace_id
    )
    _assert(missing_result_trace is not None, "missing result trace missing")
    _assert(
        missing_result_trace.get("status") == "handoff_required",
        "missing reconciliation result with reconciliation fields should hand off",
    )
    _assert(
        missing_result_trace.get("handoff_reason") == "reconciliation_result_invalid",
        "missing reconciliation result handoff reason mismatch",
    )

    empty_result_trace_id = f"rt_{uuid4().hex}"
    redis_runtime.sync_recovery_trace(
        recovery_trace_id=empty_result_trace_id,
        payload={
            "recovery_trace_id": empty_result_trace_id,
            "run_id": run_id,
            "bot_id": bot_id,
    ...
    runtime.run_once()
    empty_result_trace = redis_runtime.get_recovery_trace(
        recovery_trace_id=empty_result_trace_id
    )
    _assert(empty_result_trace is not None, "empty result trace missing")
    _assert(
        empty_result_trace.get("status") == "handoff_required",
        "empty reconciliation result should trigger manual handoff",
    )
    _assert(
        empty_result_trace.get("handoff_reason") == "reconciliation_result_invalid",
        "empty reconciliation result handoff reason mismatch",
    )
    ...
    redis_runtime.sync_recovery_trace(
        ...
        payload={
            ...
            "residual_exposure_quote": "0",
            "reconciliation_result": "matched",
            "reconciliation_open_order_count": "bad",
            "reconciliation_residual_exposure_quote": "oops",
            "reconciliation_observed_at": "not-a-timestamp",
            "created_at": _iso(datetime.now(UTC) - timedelta(seconds=2)),
            "updated_at": _iso(datetime.now(UTC) - timedelta(seconds=2)),
        },
    )
    runtime.run_once()
    invalid_result_trace = redis_runtime.get_recovery_trace(
        recovery_trace_id=invalid_result_trace_id
    )
    _assert(invalid_result_trace is not None, "invalid result trace missing")
    _assert(
        invalid_result_trace.get("status") == "handoff_required",
        "invalid reconciliation result fields should trigger manual handoff",
    )
    _assert(
        invalid_result_trace.get("handoff_reason") == "reconciliation_result_invalid",
        "invalid reconciliation result handoff reason mismatch",
    )

    future_observed_at_trace_id = f"rt_{uuid4().hex}"
    redis_runtime.sync_recovery_trace(
        recovery_trace_id=future_observed_at_trace_id,
        payload={
            "recovery_trace_id": future_observed_at_trace_id,
            "run_id": run_id,
            "bot_id": bot_id,
            ...
            "reconciliation_open_order_count": 0,
            "reconciliation_residual_exposure_quote": "0",
            "reconciliation_observed_at": _iso(datetime.now(UTC) + timedelta(seconds=30)),
            "created_at": _iso(datetime.now(UTC) - timedelta(seconds=2)),
            "updated_at": _iso(datetime.now(UTC) - timedelta(seconds=2)),
        },
    )
    runtime.run_once()
    future_observed_at_trace = redis_runtime.get_recovery_trace(
        recovery_trace_id=future_observed_at_trace_id
    )
    _assert(future_observed_at_trace is not None, "future observed_at trace missing")
    _assert(
        future_observed_at_trace.get("status") == "handoff_required",
        "future observed_at should trigger manual handoff",
    )
    _assert(
        future_observed_at_trace.get("handoff_reason") == "reconciliation_result_invalid",
        "future observed_at handoff reason mismatch",
    )

    invalid_numeric_fields_trace_id = f"rt_{uuid4().hex}"
    redis_runtime.sync_recovery_trace(
        recovery_trace_id=invalid_numeric_fields_trace_id,
        payload={
            "recovery_trace_id": invalid_numeric_fields_trace_id,
            "run_id": run_id,
            "bot_id": bot_id,
            ...
            "residual_exposure_quote": "0",
            "reconciliation_result": "matched",
            "reconciliation_open_order_count": True,
            "reconciliation_residual_exposure_quote": "-1",
            "reconciliation_observed_at": _iso(datetime.now(UTC)),
            "created_at": _iso(datetime.now(UTC) - timedelta(seconds=2)),
            "updated_at": _iso(datetime.now(UTC) - timedelta(seconds=2)),
        },
    ...
    runtime.run_once()
    invalid_numeric_fields_trace = redis_runtime.get_recovery_trace(
        recovery_trace_id=invalid_numeric_fields_trace_id
    )
    _assert(invalid_numeric_fields_trace is not None, "invalid numeric fields trace missing")
    _assert(
        invalid_numeric_fields_trace.get("status") == "handoff_required",
        "bool open_order_count or negative residual should trigger manual handoff",
    )
    _assert(
        invalid_numeric_fields_trace.get("handoff_reason") == "reconciliation_result_invalid",
    ...
    matched_failed_status_trace_id = f"rt_{uuid4().hex}"
    redis_runtime.sync_recovery_trace(
        recovery_trace_id=matched_failed_status_trace_id,
        payload={
            "recovery_trace_id": matched_failed_status_trace_id,
            "run_id": run_id,
            "bot_id": bot_id,
            ...
            "reconciliation_observed_at": _iso(datetime.now(UTC)),
            "reconciliation_observed_order_statuses": [
                {"order_id": "ord-failed-match-1", "status": "failed"}
            ],
            "created_at": _iso(datetime.now(UTC)),
            "updated_at": _iso(datetime.now(UTC)),
        },
    ...
    runtime.run_once()
    matched_failed_status_trace = redis_runtime.get_recovery_trace(
        recovery_trace_id=matched_failed_status_trace_id
    )
    _assert(
        matched_failed_status_trace is not None,
        "matched failed status trace missing",
    )
    _assert(
        matched_failed_status_trace.get("status") == "handoff_required",
        "matched reconciliation with terminal failure statuses should hand off",
    )
    _assert(
        matched_failed_status_trace.get("handoff_reason")
        == "reconciliation_terminal_failure_status_conflict",
        "matched failed status handoff reason mismatch",
    )
    ...
    terminal_success_status_trace_id = f"rt_{uuid4().hex}"
    redis_runtime.sync_recovery_trace(
        recovery_trace_id=terminal_success_status_trace_id,
        payload={
            "recovery_trace_id": terminal_success_status_trace_id,
            "run_id": run_id,
            "bot_id": bot_id,
            ...
            "reconciliation_mismatch_streak": 1,
            "reconciliation_observed_order_statuses": [
                {"order_id": "ord-filled-1", "status": "filled"}
            ],
            "created_at": _iso(datetime.now(UTC) - timedelta(seconds=2)),
            "updated_at": _iso(datetime.now(UTC)),
        },
    ...
    runtime.run_once()
    terminal_success_status_trace = redis_runtime.get_recovery_trace(
        recovery_trace_id=terminal_success_status_trace_id
    )
    _assert(
        terminal_success_status_trace is not None,
        "terminal success status trace missing",
    )
    _assert(
        terminal_success_status_trace.get("status") == "handoff_required",
        "terminal success statuses with reported open orders should trigger manual handoff",
    )
    _assert(
        terminal_success_status_trace.get("handoff_reason")
