...
def _set_runtime_spec(store, run: dict[str, object], runtime_spec: dict[str, object]) -> None:
    assigned = store.bot_details[str(run['bot_id'])]['assigned_config_version']
    config_scope = str(assigned['config_scope'])
    version_no = int(assigned['version_no'])
    for version in store.config_versions[config_scope]:
        ...
        return
    raise AssertionError('runtime config version not found in mutable sample store')
...
def main() -> None:
    ...
    multi_store, multi_run, multi_runtime_spec = _running_run()
    multi_runtime_spec['candidate_exchanges'] = [base_exchange, hedge_exchange, 'coinone']
    multi_runtime_spec['exchange_balances'] = {
        base_exchange: dict(multi_runtime_spec['base_balance']),
        hedge_exchange: dict(multi_runtime_spec['hedge_balance']),
        'coinone': {
            'available_base': '1.1',
            'available_quote': '170000000',
            'is_fresh': True,
        },
    }
    _set_runtime_spec(multi_store, multi_run, multi_runtime_spec)
    multi_connector = CachedOnlyConnector()
    for exchange in multi_runtime_spec['candidate_exchanges']:
        multi_connector.snapshots[(str(exchange), market)] = _snapshot(
            exchange=str(exchange),
            market=market,
        )
    multi_result = load_arbitrage_runtime_payload(
        store=multi_store,
        connector=multi_connector,
        run=multi_run,
        redis_runtime=None,
    )
    _assert(multi_result.payload is not None, 'runtime loader should accept multi-exchange cached snapshots')
    _assert(multi_connector.cached_fetch_count == 3, 'runtime loader should read all candidate exchange snapshots')
    _assert(
        multi_result.payload['candidate_exchanges'] == [base_exchange, hedge_exchange, 'coinone'],
        'runtime loader candidate_exchanges mismatch',
    )
    _assert(
        'coinone' in multi_result.payload['orderbooks_by_exchange'],
        'runtime loader should include third exchange orderbook payload',
    )
    _assert(
        'coinone' in multi_result.payload['balances_by_exchange'],
        'runtime loader should include third exchange balance payload',
    )
    ...
    print('PASS runtime loader expands candidate_exchanges into cached snapshot and balance payloads')
