...
- arbitrage_sim_observer.py: 실제 공개 오더북을 1초 간격 기본 루프로 읽되 거래소별 fetch 주기(upbit=3초 등)를 따로 주어 모의 재정거래 빈도와 추정 수익을 JSONL/로그로 집계한다
- arbitrage_simulation_cases.py: sim observer용 거래소 쌍 정규화, 양방향 재정거래 평가, 누적 집계가 올바른지 검증한다
