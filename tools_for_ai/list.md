...
- observer_request_stats_cases.py: public REST observer용 요청 통계가 거래소별 성공/실패 비율과 latency p50/p95/p99를 올바르게 집계하는지 검증한다
...
- arbitrage_simulation_cases.py: sim observer용 거래소 쌍 정규화, pair timing gate 보정, skew diagnostic-only 모드, 양방향 재정거래 평가, reason_code breakdown 포함 누적 집계가 올바른지 검증한다
