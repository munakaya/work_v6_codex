...
- `strategy_runtime`에는 evaluation/persist/submit 카운터와 execution 모드(`simulate_success`, `simulate_failure`, `simulate_fill`, `private_stub`, `private_http`, `private_connectors`)도 포함
...
- `strategy_runtime.execution_mode=private_connectors`이면 execution path 메타데이터는 `integrated_private_connectors`로 노출되고, ready의 추가 외부 health probe는 요구하지 않는다
