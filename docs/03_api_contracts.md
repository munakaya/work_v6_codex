...
- `strategy_runtime`에는 evaluation/persist/submit 카운터와 execution 모드(`simulate_success`, `simulate_failure`, `simulate_fill`, `private_stub`)도 포함
- `strategy_runtime`에는 현재 연결된 execution adapter 이름도 포함
- `recovery_runtime`에는 active trace 처리, auto resolve, manual handoff 승격 카운터가 포함
